...
def parse_filename(filename: str) -> Dict[str, str]:
    ...
    Also detects an optional chunk size suffix, e.g. "..._chunksize4.jsonl",
    used by IncrementalToM-style sweeps that vary the chunk size.
    """
    ...
    # Extract method
    method_keywords = ["vp", "soo", "simtom", "perceptom", "dtom", "sharedevidencetom", "assemabletom", "incrementaltom", "s3ap", "dwm"]
    ...
    # Extract chunk size, e.g. "chunksize4", "chunk_size4", "chunksize_4"
    chunk_match = re.search(r"chunk[_]?size[_]?(\d+)", filename.lower())
    if chunk_match:
        info["chunk_size"] = int(chunk_match.group(1))
...
class ExperimentAnalyzer:
    ...
    def load_all(self):
        ...
        for f in sorted(jsonl_files):
            ...
            # If the filename encodes a chunk size (e.g. IncrementalToM sweeps),
            # fold it into the method label so every downstream grouping
            # (accuracy-by-method, complementarity, best-method-per-sample, ...)
            # treats each chunk size as its own condition instead of merging
            # them all into one "IncrementalToM" bucket.
            effective_method = meta["method"]
            if meta["chunk_size"] is not None:
                effective_method = f"{meta['method']}_chunk{meta['chunk_size']}"

            exp_key = f"{effective_method}_{meta['model']}"
            ...
            for r in results:
                # Preserve the original method name reported inside the row
                # (e.g. "IncrementalToM") before overriding it.
                r["base_method"] = r.get("method", meta["method"])
                if meta["chunk_size"] is not None:
                    r["chunk_size"] = meta["chunk_size"]
                    r["method"] = effective_method
                r["_exp_key"] = exp_key
                r["_filename"] = f.name
    ...
    def compute_accuracy_by_chunk_size(self) -> Tuple[List[str], List[List[Any]]]:
        """
        Compare accuracy across chunk sizes (for methods like IncrementalToM
        that were swept over multiple chunk sizes). Rows without a chunk_size
        are skipped since they aren't part of a chunk-size sweep.
        """
        chunk_stats = defaultdict(lambda: defaultdict(lambda: {"correct": 0, "total": 0}))

        for r in self.all_results:
            chunk_size = r.get("chunk_size")
            if chunk_size is None:
                continue
            base_method = r.get("base_method", r.get("method", "UNKNOWN"))
            chunk_stats[base_method][chunk_size]["total"] += 1
            chunk_stats[base_method][chunk_size]["correct"] += int(r.get("correct", 0))

        headers = ["base_method", "chunk_size", "total", "correct", "accuracy"]
        rows = []
        for base_method in sorted(chunk_stats.keys()):
            for chunk_size in sorted(chunk_stats[base_method].keys()):
                stats = chunk_stats[base_method][chunk_size]
                acc = stats["correct"] / stats["total"] if stats["total"] > 0 else 0
                rows.append([base_method, chunk_size, stats["total"], stats["correct"], f"{acc:.2%}"])
    ...
    def generate_insights_report(self) -> str:
        ...
        # 2b. Chunk Size Comparison (only populated if any file encoded a chunk size)
        chunk_headers, chunk_rows = self.compute_accuracy_by_chunk_size()
        if chunk_rows:
            lines.append("-" * 80)
            lines.append("2b. ACCURACY BY CHUNK SIZE")
            lines.append("-" * 80)
            lines.append(formatter.format_table(chunk_headers, chunk_rows))
            lines.append("")
        ...
        lines.append("(With chunk-size sweeps, 'method' here means method+chunk-size, so this")
        lines.append(" also reveals which chunk size wins where another chunk size fails.)")
        lines.append("")
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="New_higtom_1307_analysis_report.txt",
    ...
    if args.export_csv:
        ...
        # Export chunk-size comparison, if applicable
        chunk_headers, chunk_rows = analyzer.compute_accuracy_by_chunk_size()
        if chunk_rows:
            export_to_csv(f"{args.export_csv}_chunk_sizes.csv", chunk_headers, chunk_rows)
            print(f"Chunk size comparison exported to: {args.export_csv}_chunk_sizes.csv")
