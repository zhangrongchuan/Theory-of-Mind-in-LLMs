...
class SharedEvidenceToM:
    """
    Backward-compatible implementation of the shared-evidence prompt.

    The public method name is SHAREDEVIDENCETOM, and the model-facing prompts
    are standardized on SharedEvidenceToM while keeping legacy aliases for
    older result readers.
    """

    def __init__(self, llm_callable: Callable[[str], str]):
        self.llm_callable = llm_callable
        self.last_shared_evidence_prompt: Optional[str] = None
        self.last_shared_evidence: Optional[str] = None
    ...
    def build_evidence_prompt(self, sample: Dict[str, Any]) -> str:
        ...
        return f"""/no_think
         Extract the shared evidence needed for this deeply nested belief question.
         ...
         3. The shared evidence is an INTERSECTION, not a union. Output a target-object
         ...
         4. Track presence exactly: all named entrants remain present until their own
            ...
            shared evidence. Reuniting in a waiting room does not reveal earlier events.
         5. A private claim belongs in the shared evidence only if every chain agent knows
    ...
    def build_qa_prompt(self, evidence: str, sample: Dict[str, Any]) -> str:
        ...
        return f"""SharedEvidenceToM evidence:
         ...
         Answer the deeply nested belief question using only the shared evidence.
         ...
         object. Do not import any event absent from the shared evidence.

         End with exactly:
         Answer: <option letter>"""

    def run(self, sample: Dict[str, Any]) -> str:
        self.last_shared_evidence_prompt = self.build_evidence_prompt(sample)
        self.last_evidence_prompt = self.last_shared_evidence_prompt
        self.last_core_prompt = self.last_shared_evidence_prompt
        self.last_shared_evidence = self.llm_callable(self.last_shared_evidence_prompt).strip()
        self.last_evidence = self.last_shared_evidence
        self.last_core = self.last_shared_evidence
        self.last_qa_prompt = self.build_qa_prompt(self.last_shared_evidence, sample)
