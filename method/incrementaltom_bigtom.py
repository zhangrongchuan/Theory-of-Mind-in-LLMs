...
class IncrementalToMBigToM:
    ...
    def ask_intermediate_question(
        ...
        if previous_answer:
            ...
            Intermediate Answer: TRUE
            or
            Intermediate Answer: WRONG
    ...
    def extract_answer_from_response(self, response: str) -> str:
        """Extract the TRUE/WRONG answer from an intermediate response."""
        # Look for "Intermediate Answer: TRUE" or "Answer: TRUE" pattern
        match = re.search(r'(?:Intermediate Answer|Answer)[:\s]+(TRUE|WRONG)\b', response, re.IGNORECASE)
        if match:
            return match.group(1).upper()

        # Try to find TRUE or WRONG anywhere
        if re.search(r'\bTRUE\b', response, re.IGNORECASE):
            return "TRUE"
        if re.search(r'\bWRONG\b', response, re.IGNORECASE):
            return "WRONG"

        # Return full response if no clear answer
        return response.strip()

    def format_intermediate_answer(self, response: str) -> str:
        """Format the intermediate answer for context passing."""
        answer = self.extract_answer_from_response(response)
        return f"The character {'would do/have the true belief' if answer == 'TRUE' else 'would not do/have the false belief'}."
    ...
    def run(
        ...
        for i, chunk in enumerate(chunks[:-1], 1):  # Process all but the last chunk
            ...
            formatted_answer = self.format_intermediate_answer(response)
