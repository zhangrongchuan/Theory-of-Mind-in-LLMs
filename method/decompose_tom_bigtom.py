...
class DecomposeToMBigToM:
    """
    Decompose-ToM adapted for BigToM dataset.
    ...
    def __init__(self, llm_callable, max_recursion_depth: int = 3):
        """
        Initializes the Decompose-ToM inference algorithm for BigToM.

        :param llm_callable: A function that takes a prompt string and returns a text response.
        :param max_recursion_depth: Maximum depth for recursive ToM reasoning.
        """
    ...
    def identify_subject(self, question: str) -> Optional[str]:
        ...
        """
        prompt = f"""
        Analyze the following question and identify the specific agent or subject whose perspective, belief, or knowledge is being queried.
    ...
    def reframe_question(self, question: str, subject: str) -> str:
        ...
        Example: "What will John do?" -> "What will you do?"
        """
        prompt = f"""
        Reframe the following Theory of Mind question to be a direct question from the perspective of the subject: {subject}.
        Remove references to their own belief.
    ...
    def update_world_model(self, story: str, subject: str) -> str:
        ...
        Read the following narrative. Based ONLY on what {subject} observed or was told, reconstruct the narrative from their perspective.
        Omit any events or information that {subject} is unaware of (e.g., things that happened before they entered or while they were absent).

        Original Narrative:
        {story}

        {subject}'s World Model (Observed Narrative):"""

        return self.llm(prompt).strip()

    def knowledge_availability(self, world_model: str, question: str) -> bool:
        """
        Step 4: Checks if the subject's updated world model contains enough information to answer the reframed question.
        """
        prompt = f"""
    ...
    def run(
        ...
        if current_depth >= self.max_depth:
            return self.direct_qa(story, question, true_answer, wrong_answer)
        ...
        # Recursive Call:
        # If the reframed question STILL contains a ToM query, recursively decompose
        return self.run(
            agent_world_model,
            reframed_question,
            true_answer,
