...
class IncrementalToM:
    ...
    def __init__(self, llm_callable: Callable[[str], str], chunk_size: int = 3):
        ...
        self.last_intermediate_prompts: List[str] = []
        self.last_intermediate_responses: List[str] = []
        self.last_intermediate_answers: List[Tuple[int, str]] = []
        self.last_final_prompt: Optional[str] = None
        self.last_final_response: Optional[str] = None
    ...
    def ask_intermediate_question(
        ...
        self.last_intermediate_prompts.append(prompt)
    ...
    def run(
        ...
        self.last_intermediate_prompts = []
        self.last_intermediate_responses = []
        self.last_intermediate_answers = []
        ...
        if len(sentences) <= n:
            ...
            Answer: <option letter>"""
            self.last_final_prompt = prompt
            self.last_final_response = self.llm_callable(prompt)
            return self.last_final_response
        ...
        for i, chunk in enumerate(chunks[:-1], 1):  # Process all but the last chunk
            ...
            self.last_intermediate_responses.append(response)
            ...
            self.last_intermediate_answers.append((i, formatted_answer))
        ...
        self.last_final_response = final_response
